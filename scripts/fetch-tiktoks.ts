...
function wait(delay: number) {
	return new Promise<void>((resolve, _reject) => setTimeout(() => {
...
async function fetchAndSaveMetadata(videoURLs: string[], head = false) {
	...
	console.log(`Fetching metadata for ${startIdx} - ${endIdx} of ${videoURLs.length} videos`);
	const promises = videoURLs.filter((_url, i) => i >= startIdx && i < endIdx).map((url, i) => fetchData(url));
